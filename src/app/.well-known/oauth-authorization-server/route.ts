 ...
 *   - `token_endpoint_auth_methods_supported: ["none"]` together with
 *     `client_id_metadata_document_supported` — Claude falls back to Dynamic
 *     Client Registration unless it sees BOTH, so advertising one without the
 *     other quietly costs you the better path.
 ...
 *
 * Deliberately absent: `plain` as a challenge method (removed in OAuth 2.1 and
 * pointless — the challenge would be the verifier), and the implicit and
