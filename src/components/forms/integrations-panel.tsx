...
import { CardShell, ComingSoonCards, SheetsGlyph } from "@/components/integrations/cards"
...
export function IntegrationsPanel({
  ...
  function onToggle(next: boolean) {
    startTransition(async () => {
      const res = next ? await enableFormSheet(formId) : await pauseFormSheet(formId)
      if (res.success) {
        showToast(next ? "This form will sync to Google Sheets" : "Sync paused for this form", {
          type: "success",
        })
        router.refresh()
      } else {
  ...
  if (!configured) {
    footerLead = <span className="text-xs text-muted-foreground">Not available</span>
  } else if (!connected) {
    footerLead = (
    ...
    footerLead = <span className="text-xs text-muted-foreground">Sheet created on first response</span>
  } else {
      ...
      <CardShell>
        ...
        {configured && connected ? (
          <div className="mt-2.5 flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
            <span className="truncate">Connected as {connection!.accountEmail}</span>
            <Link
              href="/integrations"
