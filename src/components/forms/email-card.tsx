...
import { saveEmailNotification, removeEmailNotification } from "@/lib/actions/email"
...
export function EmailCard({
  ...
  const [includeAnswers, setIncludeAnswers] = React.useState(notification?.includeAnswers ?? true)
  const [enabled, setEnabled] = React.useState(notification?.enabled ?? true)

  function onSave() {
    startTransition(async () => {
      const res = await saveEmailNotification(formId, { recipients, includeAnswers, enabled })
      if (res.success) {
  ...
  return (
    <>
      <CardShell>
        ...
        <h3 className="mt-3 text-sm font-semibold text-foreground">Email notifications</h3>
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-border pt-3">
          ...
          <Button variant="outline" size="sm" disabled={!configured} onClick={() => setOpen(true)}>
            Configure
          </Button>
        </div>
      </CardShell>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4">
            <div>
              <label htmlFor="email-recipients" className="text-sm font-medium text-foreground">
                Recipients
              </label>
              <p className="mb-1.5 text-xs text-muted-foreground">One email per line.</p>
              <textarea
                id="email-recipients"
                value={recipients}
            ...
            <label className="flex items-center justify-between gap-4">
              <span>
                <span className="block text-sm font-medium text-foreground">Include answers</span>
                <span className="block text-xs text-muted-foreground">
                  Put the submitted answers in the email body.
                </span>
              </span>
              <Switch checked={includeAnswers} disabled={pending} onCheckedChange={setIncludeAnswers} />
            </label>

            <label className="flex items-center justify-between gap-4 border-t border-border pt-4">
              <span>
                <span className="block text-sm font-medium text-foreground">Send notifications</span>
                <span className="block text-xs text-muted-foreground">
