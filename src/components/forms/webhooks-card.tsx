...
export function WebhooksCard({
  ...
  function onAdd() {
    if (!url.trim()) return
    startTransition(async () => {
      const res = await addWebhook(formId, { url, secret: secret || undefined })
      if (res.success) {
  ...
  function onTest(id: string) {
    ...
    startTransition(async () => {
      ...
      setTestingId(null)
      if (res.success) {
        showToast(`Test delivered${res.status ? ` (${res.status})` : ""}`, { type: "success" })
      } else {
        showToast("Test failed", {
          type: "error",
          description: res.error ?? (res.status ? `Endpoint returned ${res.status}` : undefined),
        })
      }
    })
  }
    ...
    <>
      <CardShell>
        ...
        <p className="mt-1 flex-1 text-sm text-muted-foreground">
          POST each new submission to your own endpoint, optionally signed for verification.
        </p>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader>
            ...
            <SheetDescription>
              Each new submission is sent as JSON. Add a secret to receive a signed{" "}
              <span className="font-mono">X-MakingFlow-Signature</span> header.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-4">
                  ...
                  <li key={w.id} className="flex items-center gap-3 py-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center gap-2">
                        {w.hasSecret ? (
                          ...
                          <span className="text-[11px] text-muted-foreground">Unsigned</span>
                        )}
                        <button
                          type="button"
                          onClick={() => onTest(w.id)}
                    ...
                    <Switch
                      ...
                      onCheckedChange={(next) =>
                        run(() => toggleWebhook(w.id, next), next ? "Webhook enabled" : "Webhook paused")
                      }
                    />
                    <Button
                      variant="ghost"
                      ...
                      onClick={() => run(() => removeWebhook(w.id), "Webhook removed")}
                    >
                      <Icon name="delete" />
                    </Button>
                  </li>
            ...
            <div className="mt-4 space-y-2 border-t border-border pt-4">
              ...
              <Input
                ...
                disabled={pending}
              />
              <Button onClick={onAdd} disabled={pending || !url.trim()} className="w-full">
                <Icon name="plus" />
                Add endpoint
              </Button>
            </div>
