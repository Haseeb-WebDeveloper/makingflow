import Link from "next/link"

export function Hero() {
  return (
    <section className="relative w-full overflow-hidden">
      {/* A whisper of brand light behind the headline, nothing loud. */}
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[420px] bg-[radial-gradient(60%_100%_at_50%_0%,color-mix(in_oklab,var(--primary)_8%,transparent),transparent)]"
      />

      <div className="mx-auto max-w-5xl px-5 pt-20 text-center sm:px-8 sm:pt-28">
        <span className="lp-rise inline-flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1 text-xs font-medium tracking-wide text-muted-foreground">
          <span className="size-1.5 rounded-full bg-primary" />
          AI form builder
        </span>

        <h1
          className="lp-rise mx-auto mt-6 max-w-3xl font-sebenta text-[2.75rem] font-bold leading-[1.04] tracking-tight text-foreground sm:text-7xl"
          style={{ animationDelay: "60ms" }}
        >
          Build forms that think.
        </h1>

        <p
          className="lp-rise mx-auto mt-6 max-w-xl text-pretty text-base leading-relaxed text-muted-foreground sm:text-lg"
          style={{ animationDelay: "140ms" }}
        >
          Tell MakingFlow what you need in plain words. It builds the form,
          adapts it to each person who fills it out, and hands back clean answers
          you can actually use.
        </p>

        <div
          className="lp-rise mx-auto mt-9 flex w-full flex-col items-center gap-3 sm:w-auto sm:flex-row sm:justify-center"
          style={{ animationDelay: "220ms" }}
        >
          <Link
            href="/auth/signup"
            className="inline-flex h-11 w-full items-center justify-center rounded-md bg-foreground px-6 text-sm font-medium text-background transition-colors hover:bg-foreground/90 sm:w-auto"
          >
            Start for free
          </Link>
          <Link
            href="#how"
            className="inline-flex h-11 w-full items-center justify-center rounded-md border border-border px-6 text-sm font-medium text-foreground transition-colors hover:bg-muted sm:w-auto"
          >
            See how it works
          </Link>
        </div>

        <p
          className="lp-rise mt-4 text-xs text-muted-foreground"
          style={{ animationDelay: "300ms" }}
        >
          No credit card. Your first forms are on us.
        </p>
      </div>

      {/* Real product: the builder, mid-thought. */}
      <div className="mx-auto mt-14 max-w-5xl px-5 pb-20 sm:px-8 sm:pb-28">
        <div
          className="lp-rise overflow-hidden rounded-xl border border-border bg-background shadow-lg"
          style={{ animationDelay: "360ms" }}
        >
          <BuilderTopBar />
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr]">
            <ChatPanel />
            <FormCanvas />
          </div>
        </div>
      </div>
    </section>
  )
}

function BuilderTopBar() {
  return (
    <div className="flex items-center gap-2 border-b border-border bg-muted/40 px-4 py-2.5">
      <span className="size-2.5 rounded-full bg-border" />
      <span className="size-2.5 rounded-full bg-border" />
      <span className="size-2.5 rounded-full bg-border" />
      <span className="ml-3 truncate font-sebenta text-sm font-semibold text-foreground">
        Employee feedback
      </span>
      <span className="ml-auto inline-flex items-center gap-1 rounded-full bg-success/10 px-2 py-0.5 text-[11px] font-medium text-success">
        <span className="size-1.5 rounded-full bg-success" />
        Saved
      </span>
    </div>
  )
}

function ChatPanel() {
  return (
    <div className="flex flex-col gap-4 border-b border-border p-4 lg:border-b-0 lg:border-r">
      <div className="flex items-start gap-2.5">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo/logo.svg" alt="" className="mt-0.5 size-6 shrink-0 rounded-md" />
        <p className="text-sm leading-relaxed text-muted-foreground">
          Built your feedback form. It is anonymous and takes under a minute. Ask
          for any changes.
        </p>
      </div>

      <div className="flex justify-end">
        <p className="max-w-[85%] rounded-2xl rounded-br-md bg-foreground px-3.5 py-2 text-sm text-background">
          Create a form for employee feedback
        </p>
      </div>

      <div className="mt-auto rounded-xl border border-border bg-background p-2.5">
        <p className="px-1 py-1.5 text-sm text-muted-foreground">
          Make the first question required
        </p>
        <div className="flex justify-end">
          <span className="grid size-8 place-items-center rounded-lg bg-foreground text-background">
            <svg viewBox="0 0 24 24" className="size-4" fill="none" stroke="currentColor" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
              <path d="M12 19V5M5 12l7-7 7 7" />
            </svg>
          </span>
        </div>
      </div>
    </div>
  )
}

function FormCanvas() {
  return (
    <div className="bg-canvas px-6 py-8 sm:px-10">
      <div className="mx-auto max-w-md space-y-7">
        <h2 className="font-sebenta text-xl font-bold tracking-tight text-foreground">
          How are things going?
        </h2>

        <Field delay={520} label="How happy are you at work right now?">
          <div className="flex gap-1.5">
            {[1, 2, 3, 4, 5].map((n) => (
              <span
                key={n}
                className={
                  "grid size-9 place-items-center rounded-md border text-sm " +
                  (n === 4
                    ? "border-foreground bg-foreground text-background"
                    : "border-input text-muted-foreground")
                }
              >
                {n}
              </span>
            ))}
          </div>
        </Field>

        <Field delay={640} label="What is working well?">
          <div className="h-16 rounded-md border border-input bg-background" />
        </Field>

        <Field delay={760} label="What would you change?">
          <div className="h-16 rounded-md border border-input bg-background" />
        </Field>

        <Field delay={880} label="Would you recommend us as a place to work?">
          <div className="flex gap-2">
            <span className="rounded-md border border-input px-4 py-1.5 text-sm text-foreground">
              Yes
            </span>
            <span className="rounded-md border border-input px-4 py-1.5 text-sm text-muted-foreground">
              No
            </span>
          </div>
        </Field>
      </div>
    </div>
  )
}

function Field({
  label,
  children,
  delay,
}: {
  label: string
  children: React.ReactNode
  delay: number
}) {
  return (
    <div className="lp-rise space-y-2" style={{ animationDelay: `${delay}ms` }}>
      <p className="text-sm font-medium text-foreground">{label}</p>
      {children}
    </div>
  )
}
